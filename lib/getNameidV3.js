...
const getHTMLsite = async function(site)
{
...
const getNameid = async(site)=>
{
    ...
    {
        if(errInRow>0)
            {console.log(`Cooldown activated, cooldown time: ${cd_time/1000}s `),await wait(cd_time).then(()=>console.log(`Additional attempt nr ${errInRow} started`))} 

        await getHTMLsite(site).then(
            (response)=>{
...
;(async()=>
{
    ...
    for(let i=0;i<itemsLength;i++)
    {
        ...
        items[i].nameid = await getNameid(items[i].link)      //main process, getting nameid for the item
