...
const getDataWrongArgErr = (type,logErr)=>{
...
const getData = async({type="",appid=252490,nameid=undefined,hash_name="",logErr=false,logInfo=false,req_data={}}={})=>{
    var site
    if(type==='histogram')
    {
        if(nameid==false)
    ...
    }else if(type==='pricehistory')
    {
        ...
        if(logErr)
            console.log("Error getData: Wrong 'type' argument")

        return {"success":false,"error":"Error getData: Wrong 'type' argument"}
    }
    ...
    var options = {...req_data,site}

    while(true){
        await site.getSite(options).then((res,err)=>{
            if(err)
            {
                return {"success" : false, "error" : [err]}
            }
