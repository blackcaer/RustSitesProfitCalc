...
const getDataWrongArgErr = (type,logErr)=>{ // Helper func for getData
...
async function getChunkOfSteamMarket({start=0,count=100,req_data={}}={}){
    ...
    {
        return {"success" : true, "data" : [...response.results], "info" :{ "total_count": response.total_count, "start" : response.start, "pagesize" : response.pagesize}}
    }
    else
        {
...
const getData = async({type="",appid=252490,nameid=undefined,hash_name="",logErr=false,logInfo=true,req_data={},cd_tooManyRequest_error=301000,maxTMRerrInRow=2}={})=>{ 
    // TODO dodac promise albo wywalic async w ogole
    /* Getting item data
    Better replacement for getHistogram, getPriceHistory, getPriceOverview. Handling too many requests err not explictly, 
    because it retries after 'cd_tooManyRequest_error' ms maxTMRerrInRow times*/
    var site
    // Check if the arguments are correct and setting site
    if(type==='histogram')
    {
        if(nameid==false)
            return getDataWrongArgErr(type,logErr)
    ...
    {
        if(appid==false || hash_name==false)
            {return getDataWrongArgErr(type,logErr)}
    ...
    while(true){ // getSite loop
        ...
        }catch(error) // || response.headers.htmlcode TODO
        {
            ...
            if(logInfo)
            {
                ...
                console.log(`Status code: ${tmp.statusCode}`)
            }
